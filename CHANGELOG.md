...
### Added
- 🛒 **A shop.** `workers/ecommerce` sells digital products — ebooks, or any
  file — from a static site, worldwide, on Cloudflare Pages Functions with D1,
  R2 and KV. Stripe and PayPal, VAT worked out per country from a dated rate
  table, invoices with gapless numbers and a frozen snapshot, download links
  that expire and count their uses, and an admin panel at `/ecommerce-admin/`.
  No npm packages at runtime: the deployed tree is the source tree.

  Two decisions shape the rest. Nothing trusts the browser about money — the
  checkout endpoint is sent product codes and quantities, and prices, rates and
  totals come from the database every time. And everything is safe to run twice:
  D1 has no transaction spanning requests, so instead of a rollback that does not
  exist, each step carries its own "have I already done this" — the move to
  `paid` names the states it may move from, the webhook inbox row is the lock,
...
- 🎨 **`picostore`**, a shop theme built on [Pico CSS](https://picocss.com)
  v2.1.1 (MIT, vendored): a catalogue, a product page, a basket, a checkout and
  the four pages either side of a payment. The **Buy now** button on a product
  page is a real form that works with JavaScript switched off; the basket, which
  lives in the browser by definition, is the part that needs it. Colour is from
  the Google Material palette, every pairing measured against WCAG 2.2 AA, and
  no status is signalled by colour alone. See
  [its README](templates/picostore/README.md).

- 📦 **`examples/ebook-shop`**, the two of them together as a working shop: a
  publisher with two books, VAT for eight countries, sample PDFs generated by a
  script, and `scripts/seed.sh` to fill the shop over its own API. `ssg --watch`
...
- 🎛️ **The shop panel is an admin panel now.** It was a column of tabs above a
  ...
  collapsed, per browser, with its items on the left edge in both states; the
  content keeps a readable maximum width and sits against the menu.
...
- 📉 **The binary is 50 MB again, not 600 MB.** `//go:embed all:workers` swept
  ...
  now listed one by one, with tests that fail when a new worker is missed and
